...
#else
/*  testvalue = candidate->theta*fabs(candidate->pivot) -
              current->theta*fabs(current->pivot);*/
...
STATIC int rowprim(lprec *lp, int colnr, LREAL *theta, REAL *pcol)
{
  int      i, iy, iz, pass, k;
  LREAL    f, savef;
  REAL     Hscale, Hlimit, epspivot = lp->epspivot;
  pricerec current, candidate;
  MYBOOL   isupper = !lp->is_lower[colnr];

  /* Update local value of pivot setting */
  lp->_piv_rule_ = get_piv_rule(lp);

#ifdef UseRelativePrimalPivot
#if libBLAS > 0
  /* Compute relative size of epspivot */
  i = idamax(lp->rows, pcol, 1);
  Hlimit = fabs(pcol[i]);
#else
  /* Compute relative size of epspivot */
  Hlimit = 0;
...
Retry:
  ...
  for(; pass > 0; pass--) {
    ...
    for(; i*iz <= iy*iz; i += iz) {
      ...
      if(fabs(f) < epspivot) {
        ...
        candidate.pivot = f;
        candidate.varno = i;
        k = compute_theta(lp, i, &candidate.theta, isupper, Hscale, TRUE);
  ...
  if(current.varno == 0) {
    if(lp->upbo[colnr] >= lp->infinite) {
      /* Try again if the absolute pivot threshold was higher than the relative */
      if(epspivot > lp->epspivot) {
...
doLoop:
  ...
  for(; k*iz <= iy*iz; k += iz) {
   ...
   /* Analyze relevant constraints ...
    ...
    if((rh < 0) || (rh > up) || ((rh == 0) && (eliminate == TRUE))) {
      if(rh > up)
        rh = up-rh;
                                                        ...
                                                        REAL *drow, int *nzdrow)
{
...
#endif
  LREAL    w, g, quot;
  REAL     epsvalue = lp->epsprimal, epspivot = lp->epspivot;
  pricerec current, candidate;
  ...
  for(i = lp->sum; i > 0; i--) {
    ...
    if(lp->is_basic[i] || (fabs(lp->upbo[i]) < lp->epsprimal))
      continue;
