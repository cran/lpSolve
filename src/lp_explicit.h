...
read_LP_func                  *_read_LP;
read_mps_func                 *_read_mps;
read_MPS_func                 *_read_MPS;
read_freemps_func             *_read_freemps;
read_freeMPS_func             *_read_freeMPS;
...
# if defined WIN32
  ...
  lpsolve = LoadLibrary("lpsolve51.dll");
# else
  lpsolve = dlopen("liblpsolve51.so", RTLD_LAZY);;
# endif
  return(lpsolve);
}
...
# endif
  ...
  _is_free                 = (is_free_func *) AddressOf(lpsolve, "is_free");
  ...
  _set_free                = (set_free_func *) AddressOf(lpsolve, "set_free");
