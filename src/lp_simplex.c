...
STATIC int primloop(lprec *lp, MYBOOL feasible)
{
 ...
 /* Iterate while we are successful; exit when the model is infeasible/unbounded,
  ...
  while(lp->spx_status == RUNNING) {
    ...
    if(lp->spx_trace)
      if(lastnr > 0)
      report(lp, NORMAL, "primloop: Objective at iteration %8d is " MPSVALUEMASK " (%4d: %4d %s- %4d)\n",
...
RetryCol:
    ...
    if(lp->doIterate) {
      ...
      minit = performiteration(lp, rownr, colnr, theta, primal, NULL, NULL, pcol, NULL);
...
STATIC int dualloop(lprec *lp, MYBOOL feasible)
{
 ...
 /* Refactorize the basis and set status variables */
  ...
  while(lp->spx_status == RUNNING) {
    ...
    if(lp->spx_trace)
      if(lastnr > 0)
      report(lp, NORMAL, "dualloop: Objective at iteration %8d is " MPSVALUEMASK " (%4d: %4d %s- %4d)\n",
                         get_total_iter(lp), lp->rhs[0], rownr, lastnr,
                         my_if(minit == ITERATE_MAJORMAJOR, "<","|"), colnr);
...
RetryCol:
      ...
      if((lp->Extrad != 0) && (colnr < 0) && !isPrimalFeasible(lp, lp->epsprimal)) {
       ...
       /* Problem: Check if we are dual degenerate and need to switch to the
        ...
        if(lp->total_iter == 0) {
          report(lp, NORMAL,   "Found feasibility by dual simplex at iteration    %8d\n",
                             get_total_iter(lp));
    ...
    if(lp->doIterate) {
      ...
      if(lp->justInverted)
        ...
        recompute_solution(lp, INITSOL_USEZERO);
      minit = performiteration(lp, rownr, colnr, theta, primal, prow, nzprow, pcol, NULL);
      if(minit != ITERATE_MAJORMAJOR)
        minitcount++;
    ...
    if(lp->spx_status == SWITCH_TO_PRIMAL) {
      ...
      i = invert(lp, INITSOL_USEZERO, bfpfinal);
      bfpfinal = FALSE;
#ifdef ResetMinitOnReinvert
      minit = ITERATE_MAJORMAJOR;
#endif
...
{
  int    i, j, singular_count, lost_feas_count;
