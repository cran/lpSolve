...
/* --------------------------------------------------------------------------
  ...
  Changes 5.0+:         Kjell Eikland     -  kjell.eikland@broadpark.no
                        ...
                        Peter Notebaert   -  peno@mailme.org
                        (Sensitivity analysis, New lp parser, LINDO (XLI) parser, VB/.NET interface, documentation)
...
#define UseRelativePrimalPivot    /* Use relative rather than absolute pivot */
/*#define UseRelativeDualPivot */     /* Use relative rather than absolute pivot */
/*#define Phase1DualPriceEqualities */   /* Force elimination of equality slacks */
...
#define RELEASE                  0
#define BUILD                    3
#define BFPVERSION               7       /* Checked against bfp_compatible() */
#define XLIVERSION               6       /* Checked against xli_compatible() */
...
#define PRICE_AUTOPARTIALCOLS  256    /* Detect and use data on the block structure of the model (primal) */
#define PRICE_AUTOPARTIALROWS  512    /* Detect and use data on the block structure of the model (dual) */
...
#define PRICE_AUTOMULTICOLS   4096    /* Automatically select multiple pricing (primal) */
#define PRICE_AUTOMULTIROWS   8192    /* Automatically select multiple pricing (dual) */

#define PRICE_AUTOPARTIAL        (PRICE_AUTOPARTIALCOLS + PRICE_AUTOPARTIALROWS)
#define PRICE_AUTOMULTIPLE       (PRICE_AUTOMULTICOLS + PRICE_AUTOMULTIROWS)
...
#define DEF_EPSPRIMAL      1.0e-10  /* 1.0e-10 For rounding primal/RHS values to 0 */
#define DEF_EPSDUAL        1.0e-09  /* 1.0e-09 For rounding reduced costs to 0 */
#define DEF_EPSPIVOT       2.0e-07  /* 1.0e-06 Pivot reject threshold */
...
typedef MYBOOL (__WINAPI set_column_func)(lprec *lp, int col_no, REAL *column);
typedef MYBOOL (__WINAPI set_columnex_func)(lprec *lp, int col_no, int count, REAL *column, int *rowno);
...
typedef MYBOOL (__WINAPI set_row_func)(lprec *lp, int row_no, REAL *row);
typedef MYBOOL (__WINAPI set_rowex_func)(lprec *lp, int row_no, int count, REAL *row, int *colno);
...
typedef char * (__WINAPI explainfunc)(lprec *lp, char *format, ...);
...
struct _lprec
{
  ...
  int       freevars;           /* (placeholder, not yet used) */
  ...
  REAL      *OF_main;           /* Unused pointer - Placeholder for OF not part of B */
...
MYBOOL __EXPORT_TYPE __WINAPI set_row(lprec *lp, int row_no, REAL *row);
MYBOOL __EXPORT_TYPE __WINAPI set_rowex(lprec *lp, int row_no, int count, REAL *row, int *colno);
...
MYBOOL __EXPORT_TYPE __WINAPI set_column(lprec *lp, int col_no, REAL *column);
MYBOOL __EXPORT_TYPE __WINAPI set_columnex(lprec *lp, int col_no, int count, REAL *column, int *rowno);
...
STATIC MYBOOL performiteration(lprec *lp, int row_nr, int varin, LREAL theta, MYBOOL primal, REAL *prow, int *nzprow, REAL *pcol, int *nzpcol);
STATIC void transfer_solution_var(lprec *lp, int uservar);
